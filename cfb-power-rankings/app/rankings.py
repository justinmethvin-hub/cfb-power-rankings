...
def fetch_season_stats(year: int):
    # Use CollegeFootballData season stats endpoints
    # We'll focus on offense/defense yards/points per play.
    ...
    # Select and rename core metrics if present
    mapping = {
        "Yards Per Play": "Off_YPP",
        "Points Per Play": "Off_PPP",
    }
    df_off = df_off[[c for c in mapping if c in df_off.columns]].rename(columns=mapping)

    mapping_def = {
        "Opp Yards Per Play": "Def_YPP",
        "Opp Points Per Play": "Def_PPP",
    }
...
def compute_power_scores(df: pd.DataFrame, weights=None):
    if weights is None:
        weights = {
            "Off_YPP": 0.25,
            "Off_PPP": 0.25,
            "Def_YPP": -0.25,  # negative: lower is better
            "Def_PPP": -0.25,
        }
    # Z-score each selected column
    zcols = {}
    for col in weights.keys():
        if col in df.columns:
            zcols[col] = zscore(df[col])
        else:
            zcols[col] = pd.Series(0, index=df.index)
